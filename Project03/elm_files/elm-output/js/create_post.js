...
var author$project$CreatePost$encodeRequest = function (v) {
	return elm$json$Json$Encode$object(
		_List_fromArray(
			[
				...
				_Utils_Tuple2(
				'topic',
				elm$json$Json$Encode$string(v.topic))
			]));
};
var author$project$CreatePost$errorHandler = F2(
...
var author$project$CreatePost$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'Title':
				var string = msg.a;
					...
					elm$http$Http$post(
						{
							body: elm$http$Http$jsonBody(
								author$project$CreatePost$encodeRequest(model)),
							expect: elm$http$Http$expectString(author$project$CreatePost$PostResponce),
							url: '/e/alkersho/forum/create/'
						}));
			default:
				var result = msg.a;
				if (result.$ === 'Ok') {
					...
					if (val === '') {
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{error: 'Success'}),
							elm$browser$Browser$Navigation$load('/e/alkersho/forum/'));
					} else {
						return _Utils_Tuple2(
							_Utils_update(
								model,
					...
					var val = result.a;
					return _Utils_Tuple2(
						A2(author$project$CreatePost$errorHandler, model, val),
						elm$core$Platform$Cmd$none);
				}
		}
	});
...
var author$project$CreatePost$Topic = function (a) {
	return {$: 'Topic', a: a};
};
...
var author$project$CreatePost$errorView = function (model) {
	var _n0 = model.error;
	if (_n0 === '') {
...
var author$project$CreatePost$topicsList = function (list) {
	if (!list.b) {
		return _List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						elm$html$Html$text('No Topics Available now!')
					]))
			]);
	} else {
		if (!list.b.b) {
...
var author$project$CreatePost$view = function (model) {
	return A2(
		rundis$elm_bootstrap$Bootstrap$Grid$container,
		_List_Nil,
		_List_fromArray(
			[
				author$project$CreatePost$errorView(model),
				A2(
				...
				_List_fromArray(
					[
						rundis$elm_bootstrap$Bootstrap$Form$Select$onChange(author$project$CreatePost$Topic)
					]),
				author$project$CreatePost$topicsList(model.topics)),
				A2(
				rundis$elm_bootstrap$Bootstrap$Form$label,
				_List_Nil,
				_List_fromArray(
				...
				_List_fromArray(
					[
						rundis$elm_bootstrap$Bootstrap$Form$Input$onInput(author$project$CreatePost$Title),
						rundis$elm_bootstrap$Bootstrap$Form$Input$value(model.title),
						rundis$elm_bootstrap$Bootstrap$Form$Input$placeholder('Title')
					])),
				A2(
				rundis$elm_bootstrap$Bootstrap$Form$label,
				_List_Nil,
				...
				_List_fromArray(
					[
						...
						rundis$elm_bootstrap$Bootstrap$Button$onClick(author$project$CreatePost$Send)
					]),
				_List_fromArray(
					[
...
var author$project$CreatePost$main = elm$browser$Browser$element(
	{
		init: author$project$CreatePost$init,
		subscriptions: function (_n0) {
			return elm$core$Platform$Sub$none;
		},
		update: author$project$CreatePost$update,
		view: author$project$CreatePost$view
	});
_Platform_export({'CreatePost':{'init':author$project$CreatePost$main(
	A2(
